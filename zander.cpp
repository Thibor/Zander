...
#define Interior Convert(0x007E7E7E7E7E7E00,U64)
#define Boundary (~Interior)
#define WhiteArea Convert(0x00000000FFFFFFFF,U64)
#define BlackArea (~WhiteArea)
#define LightArea Convert(0x55AA55AA55AA55AA,U64)
#define DarkArea (~LightArea)
#define FileA Convert(0x0101010101010101,U64)
#define Line0 Convert(0x00000000000000FF,U64)
...
const U64 File[8] = { FileA,FileA << 1,FileA << 2,FileA << 3,FileA << 4,FileA << 5,FileA << 6,FileA << 7 };
const U64 Line[8] = { Line0,(Line0 << 8),(Line0 << 16),(Line0 << 24),(Line0 << 32),(Line0 << 40),(Line0 << 48),(Line0 << 56) };
...
typedef struct {
	U64 attacks[64];
	U64 key, pawn_key, eval_key, all_att[2], pawn_att[2], passer, xray;
...
int hashSize = initial_hash_size;
...
int PVN, Infinite, MoveTime, SearchMoves, SMPointer, Ponder, Searching, Bad, Change, Early, Easy;
#define EasyMargin 4
...
S64 InfoTime, CurrTime;
uint16 SMoves[256];

inline unsigned long lsb(U64 x);
inline unsigned long msb(U64 x);
inline int popcnt(U64 x);
int popcount(U64 x);
U64 BMagicHash(int i, U64 mask);
U64 RMagicHash(int i, U64 mask);
void init();
uint16 rand16();
U64 random();
void init_keys();
...
inline int is_defence(int move, U64 target);
...
void send_best_move();
void ParsePosition(char string[]);
void ParseGo(char string[]);
...
static void ResetInfo() {
	...
	info.nodesLimit = 0;
	info.timeLimit = 0;
	info.timeStart = (U64)GetTickCount64();
}
...
U64 BMagicHash(int i, U64 occ) {
	...
	if (File(i) < 7 && Rank(i) < 7) {
		occ |= File[7] | Line[7];
		for (j = i + 9; ((j & 7) < 7) && ((occ & ((U64)1 << j)) == 0); j += 9) free |= (U64)1 << j;
		free |= (U64)1 << j;
	}
	if (File(i) > 0 && Rank(i) < 7) {
		occ |= File[0] | Line[7];
		for (j = i + 7; ((j & 7) > 0) && ((occ & ((U64)1 << j)) == 0); j += 7) free |= (U64)1 << j;
		free |= (U64)1 << j;
	}
	if (File(i) > 0 && Rank(i) > 0) {
		occ |= File[0] | Line[0];
		for (j = i - 9; ((j & 7) > 0) && ((occ & ((U64)1 << j)) == 0); j -= 9) free |= (U64)1 << j;
		free |= (U64)1 << j;
	}
	if (File(i) < 7 && Rank(i) > 0) {
		occ |= File[7] | Line[0];
		for (j = i - 7; ((j & 7) < 7) && ((occ & ((U64)1 << j)) == 0); j -= 7) free |= (U64)1 << j;
		free |= (U64)1 << j;
	}
	return free;
}

U64 RMagicHash(int i, U64 occ) {
	U64 free = 0;
	int j;
	if (File(i) < 7) {
		occ |= File[7];
		for (j = i + 1; ((j & 7) < 7) && ((occ & ((U64)1 << j)) == 0); j++) free |= (U64)1 << j;
		free |= (U64)1 << j;
	}
	if (File(i) > 0) {
		occ |= File[0];
		for (j = i - 1; ((j & 7) > 0) && ((occ & ((U64)1 << j)) == 0); j--) free |= (U64)1 << j;
		free |= (U64)1 << j;
	}
	if (Rank(i) < 7) {
		occ |= Line[7];
		for (j = i + 8; (j < 56) && ((occ & ((U64)1 << j)) == 0); j += 8) free |= (U64)1 << j;
		free |= (U64)1 << j;
	}
	if (Rank(i) > 0) {
		occ |= Line[0];
		for (j = i - 8; (j > 7) && ((occ & ((U64)1 << j)) == 0); j -= 8) free |= (U64)1 << j;
		free |= (U64)1 << j;
	}
	return free;
}
...
uint16 rand16() {
	seed = (seed * Convert(6364136223846793005, U64)) + Convert(1442695040888963407, U64);
...
#else
	key = Convert(rand16(), U64); key <<= 16;
	key |= Convert(rand16(), U64); key <<= 16;
	key |= Convert(rand16(), U64); key <<= 16;
	key |= Convert(rand16(), U64);
#endif
	return key;
}
...
void init_hash(int mbSize) {
	...
	if (mbSize > 1024)
		mbSize = 1024;
	int recSize = sizeof(GEntry);
	hashSize = 1;
	while (hashSize * recSize <= mbSize * 1000000)
		hashSize <<= 1;
	hash_mask = (hashSize - 1) & ~3;
	free(Hash);
	Hash = (GEntry*)calloc(hashSize, recSize);
	memset(Hash, 0, hashSize * sizeof(GEntry));
}
...
void do_move(int move) {
	...
	GData* Next;
	U64 u, updated, mask_from, mask_to, occ;
	to = To(move);
	Next = Current + 1;
	Next->ep_square = 0;
...
int* gen_checks(U64 capture_target, int* list) { // this function doesn't generate ALL checks
	int king, from;
	U64 u, v, occ, target, b_target, r_target, clear, xray;
...
int see(int move) {
	...
	U64 clear, def, att, occ, b_area, r_slider_att, b_slider_att, r_slider_def, b_slider_def, r_area, u, new_att, my_bishop, opp_bishop;
...
void eval_pawns() {
	...
	U64 u, v, pawns, border_w, straight_w, center_w, area_w, border_b, straight_b, center_b, area_b, mask;
...
void evaluate() {
	...
	U64 u, v, pw_att, pb_att, w_att, b_att, att, target, up_target, threat, free, way;
	...
	for (u = Board->bb[WhiteKnight]; True(u); u ^= way) {
		...
		score += popcount(free & att & Forward[0][Rank(sq)]) * KnightMobility;
	}
	for (u = Board->bb[WhiteLight] | Board->bb[WhiteDark]; True(u); u ^= way) {
		...
		if (pb_att & way) continue;
		if (att & up_target) score += PieceUPAttack;
		score += popcount(free & att & Forward[0][Rank(sq)]) * BishopMobility; // simple "safe forward" mobility appears to be the best:)
	}
	for (u = Board->bb[WhiteRook]; True(u); u ^= way) {
		...
		if (pb_att & way) continue;
		if (att & up_target) score += PieceUPAttack;
		score += popcount(free & att) * RookMobility;
	...
	for (u = Board->bb[WhiteQueen]; True(u); u ^= way) {
		...
		if (pb_att & way) continue;
		if (att & up_target) score += PieceUPAttack;
		score += popcount(free & att) * QueenMobility;
	}
	...
	for (u = Board->bb[BlackKnight]; True(u); u ^= way) {
		...
		if (att & up_target) score -= PieceUPAttack;
		score -= popcount(free & att & Forward[1][Rank(sq)]) * KnightMobility;
	}
	for (u = Board->bb[BlackLight] | Board->bb[BlackDark]; True(u); u ^= way) {
		...
		if (pw_att & way) continue;
		if (att & up_target) score -= PieceUPAttack;
		score -= popcount(free & att & Forward[1][Rank(sq)]) * BishopMobility;
	}
	...
	Current->all_att[White] = w_att;
	Current->all_att[Black] = b_att;
	score += popcount(pw_att & (Board->bb[White] ^ Board->bb[WhitePawn] ^ Board->bb[WhiteKing])) * PieceOutpost;
	score -= popcount(pb_att & (Board->bb[Black] ^ Board->bb[BlackPawn] ^ Board->bb[BlackKing])) * PieceOutpost;
	score -= popcount((~w_att) & (Board->bb[White] ^ Board->bb[WhitePawn] ^ Board->bb[WhiteKing])) * PieceHanging;
	score += popcount((~b_att) & (Board->bb[Black] ^ Board->bb[BlackPawn] ^ Board->bb[BlackKing])) * PieceHanging;
	if (Current->material & FlagUnusualMaterial) {
		int dp, wmin, bmin, wr, br, wq, bq, phase;
		dp = popcount(Board->bb[WhitePawn]) - popcount(Board->bb[BlackPawn]);
		wmin = popcount(Board->bb[WhiteKnight] | Board->bb[WhiteLight] | Board->bb[WhiteDark]);
		bmin = popcount(Board->bb[BlackKnight] | Board->bb[BlackLight] | Board->bb[BlackDark]);
		wr = popcount(Board->bb[WhiteRook]);
		br = popcount(Board->bb[BlackRook]);
		wq = popcount(Board->bb[WhiteQueen]);
		bq = popcount(Board->bb[BlackQueen]);
...
void hash_high(int value, int depth) {
	...
	for (i = 0, best = Entry = Hash + (High32(Current->key) & hash_mask); i < 4; i++, Entry++) {
		if (Entry->key == Low32(Current->key)) {
			...
			score = (Convert(Entry->date, U64) << 32) | Convert(Entry->high_depth, U64);
		}
		else score = (Convert(Entry->date, U64) << 32) | Convert(Max(Entry->high_depth, Entry->low_depth), U64);
		if (score < best_score) {
			best_score = score;
			best = Entry;
		}
...
void hash_low(int move, int value, int depth) {
	...
	for (i = 0, best = Entry = Hash + (High32(Current->key) & hash_mask); i < 4; i++, Entry++) {
		if (Entry->key == Low32(Current->key)) {
			...
			score = (Convert(Entry->date, U64) << 32) | Convert(Entry->low_depth, U64);
		}
		else score = (Convert(Entry->date, U64) << 32) | Convert(Max(Entry->high_depth, Entry->low_depth), U64);
		if (score < best_score) {
			best_score = score;
...
void hash_exact(int move, int value, int depth) {
	...
	for (i = 0, best = PVEntry = PVHash + (High32(Current->key) & pv_hash_mask); i < pv_cluster_size; i++, PVEntry++) {
		...
		score = (Convert(PVEntry->date, U64) << 32) | Convert(PVEntry->depth, U64);
		if (score < best_score) {
			best_score = score;
			best = PVEntry;
		}
...
void root() {
	...
	if (PVN > 1) {
		...
		for (i = 0; Current->moves[i] != 0; i++)
			MultiPV[i] = Current->moves[i];
	}
	...
	for (depth = 18; depth < DepthLimit; depth += 2) {
		...
		LastDepth = depth;
		LastTime = GetTickCount64() - info.timeStart;
		previous = value;
		check_time(0);
	}
	Searching = 0;
	if (False(Infinite) || DepthLimit < 128)
		send_best_move();
}
...
void check_time(int searching) {
	while (!info.stop && input()) uci();
	if (info.stop) goto jump;
	CurrTime = GetTickCount64();
	int Time = Convert(CurrTime - info.timeStart, int);
	if (info.post && Time > InfoLag && CurrTime - InfoTime > InfoDelay) {
		InfoTime = CurrTime;
		if (info_string[0]) {
...
void uci() {
	...
	if (!strcmp(input_string, "uci")) {
		...
		if (False(Searching)) ParsePosition(input_string);
	}
	else if (!memcmp(input_string, "go", 2)) {
		if (False(Searching)) ParseGo(input_string);
	}
		...
		Infinite = 0;
		if (LastTime > TimeLimit1) info.stop = true;
		if (False(Change) && LastTime * 16 > TimeLimit1 * NormalMargin) info.stop = true;
		if (True(Early) && LastTime * 16 > TimeLimit1 * EarlyMargin) info.stop = true;
		if (True(Easy) && LastTime * 16 > TimeLimit1 * EasyMargin) info.stop = true;
		if (False(Searching)) send_best_move();
	}
	...
	else if (!memcmp(input_string, "bench", 5))
		UciBench();
}
